...
- [x] 开源发布：README 专业规范重写（shields.io 磁贴徽章/结构化分区）+ LICENSE 文件 + 测试数据清理（37 个测试学生/运行产物/一次性脚本全清）

### 发布前收尾（2026-08-29 完成）
- [x] **诊断接入真题链**（用户指出诊断也是出新题）：与出题共享 web→jszkk→火花 链；无答案真题剔除；难度混排；真题不足 AI 生成补足（真机验证 real=True）
- [x] **视觉模型回落主模型**：未配置 models.vision 时自动用多模态主模型（resolve_vision_spec，6 处装配点统一）；doctor 同步
- [x] **LLM 流式修复**：GLM 长 JSON 生成非流式撞 read 超时（诊断必挂）→ 纯文本生成改流式接收，429/5xx 重试语义保留
- [x] **题集模糊去重**：题干规范化（空白/标点/全半角/运算符变体剔除）后比对，OCR 转写差异不重复收录
- [x] **测试数据清理**：37 个测试学生（含真机 QQ 号）合规删除；data/ 运行产物（日志/调研笔记/测试图片/导出）清空；一次性脚本移除（保留 docker-build.sh + evaluate.py）；根目录 botpy 日志删除
- [ ] 知识库语义检索需嵌入 key（用户提供后配置，可回退关键词，不阻塞）
