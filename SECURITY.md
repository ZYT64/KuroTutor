...
- The agent runs in a **sandboxed auto mode**: file operations are restricted to the workspace (path-traversal and symlink-escape protected), system settings are off-limits, and model endpoints are whitelisted.
- The code sandbox is **fully open**: any Python code (including imports, file I/O, subprocess) can run inside the isolated subprocess.
- Includes a built-in **web admin panel** for monitoring students, editing configuration, and managing backups.
