...
- **M2 ✅（已完成）**：个性化出题（真题优先四级链 web→jszkk→有道/火花→生成 + 函数绘图 + 判分闭环）、
  定时课堂完整（排课/自动备课/开课推送/课后闭环/应急改期）、学习周报（Word 导出+周日自动推送）、
  校本同步、自进化记忆（对话事实自动提取注入）。
- **M3 ✅（已完成）**：入学诊断（分级摸底→画像基线）、目标管理+每日打卡激励、代码沙箱（AST 白名单+隔离子进程）、
  评测集（evals/ + scripts/evaluate.py）、49 个 Agent 工具。
  公式 LaTeX 渲染受 QQ 官方 markdown 限制（公式以题库图片形式呈现）。
