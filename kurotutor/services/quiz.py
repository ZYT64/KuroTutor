...
def search_youdao(
    keyword: str, count: int = 3, *, api_key: str, secret: str = "", timeout: float = 15.0
) -> list[dict[str, Any]]:
    """有道智云「精品题库」API（6000 万 K12 题库，0.1 元/次有效搜题，注册送 50 元）。

    文本搜题：searchType=text，q 为题目关键词（≤50 字）。api_key=应用ID，secret=应用密钥
    （ai.youdao.com 创建应用获取）。v2 签名：sha256(appKey+input+salt+curtime+secret)。
    """
    import hashlib
    ...
    q = keyword
    truncate = q if len(q) <= 20 else q[:10] + str(len(q)) + q[-10:] + str(len(q))
    sign = hashlib.sha256(f"{api_key}{truncate}{salt}{curtime}{secret}".encode()).hexdigest()
    ...
    if str(data.get("errorCode")) not in ("0", ""):
        raise RuntimeError(f"有道题库错误码 {data.get('errorCode')}")
    out: list[dict[str, Any]] = []
